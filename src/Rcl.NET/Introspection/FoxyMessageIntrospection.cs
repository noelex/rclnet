...
internal unsafe class FoxyMessageIntrospection : IMessageIntrospection
{
    ...
    public FoxyMessageIntrospection(TypeSupportHandle typeSupport)
        : this(typeSupport.GetMessageTypeSupport())
    {
    }

    public FoxyMessageIntrospection(MessageTypeSupport* ts)
    {
        fixed (byte* id = TypeSupportIdentifier.Introspection)
        {
            ts = ts->Handler(ts, (sbyte*)id);
            _typesupport = (MessageMembers*)ts->Data;
            _members = (MessageMember_Foxy*)_typesupport->Members;
        }
    }

    public unsafe FoxyMessageIntrospection(MessageMembers* members)
    {
    ...
    public RosMessageBuffer CreateBuffer()
    {
        ...
        {
            var self = (FoxyMessageIntrospection)state!;
