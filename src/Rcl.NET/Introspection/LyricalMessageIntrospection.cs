...
internal unsafe class LyricalMessageIntrospection : IMessageIntrospection
{
    private readonly MessageMembers_Jazzy* _typesupport;
    private readonly MessageMember_Lyrical* _members;

    public LyricalMessageIntrospection(TypeSupportHandle typeSupport)
        : this(typeSupport.GetMessageTypeSupport())
    {
    }

    public LyricalMessageIntrospection(MessageTypeSupport* ts)
    {
        fixed (byte* id = TypeSupportIdentifier.Introspection)
        {
            ts = ts->Handler(ts, (sbyte*)id);
            _typesupport = (MessageMembers_Jazzy*)ts->Data;
            _members = (MessageMember_Lyrical*)_typesupport->Members;
        }
    }

    public LyricalMessageIntrospection(MessageMembers_Jazzy* members)
    {
    ...
    public RosMessageBuffer CreateBuffer()
    {
        ...
        {
            var self = (LyricalMessageIntrospection)state!;
